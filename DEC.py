...
if __name__ == "__main__":
    ...
    dataInput = sio.loadmat('USPS.mat')
    print(dataInput)
    x = dataInput['fea'] /255

    Knum = 5
    W2 = kneighbors_graph(x, Knum, mode='distance', include_self=True)



    y = (dataInput['gnd'].T)[0]
    print(y)
    n_clusters = len(np.unique(y))-3
    x = spectral_embedding(W2, n_components=n_clusters)
    ...
    dec.model.summary()
    t0 = time()
    dec.compile(optimizer=SGD(0.003, 0.9), loss='kld')
    ...
    xmap = dec.encoder.predict(x)
    preLabel = DPMDEC(xmap, 0.01)
    print("RBVI NMI", metrics.nmi(preLabel, y))
    preLabel = DPMDEC(xmap, 0)
    print("VI NMI", metrics.nmi(preLabel, y))
